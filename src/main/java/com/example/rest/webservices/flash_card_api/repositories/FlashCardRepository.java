...
public class FlashCardRepository implements FlashCardRepositoryInterface {
  ...
  public String createFlashCard(FlashCard flashCard) {
    ...
    try {
      ApiFuture<DocumentReference> flashCards =
          firestore.collection(PATH_NAME_FOR_FLASH_CARD_COLLECTION).add(flashCard);
      ...
      if (!flashCardId.isEmpty() || flashCardId != null) {
        ...
        throw new RuntimeException(
            "Error in creating new flash card. Flash Card Id returned empty or null");
      }
  ...
  public List<FlashCard> retrieveAllFlashCards() {
    try {
      ApiFuture<QuerySnapshot> future =
          firestore.collection(PATH_NAME_FOR_FLASH_CARD_COLLECTION).get();
  ...
  public FlashCard retrieveFlashCardById(String id) {
    try {
      ...
      ApiFuture<DocumentSnapshot> doc =
          firestore.collection(PATH_NAME_FOR_FLASH_CARD_COLLECTION).document(id).get();
  ...
  public String deleteFlashCardById(String flashcardId) throws NotFoundException {
    try {
      if (firestore.collection(PATH_NAME_FOR_FLASH_CARD_COLLECTION).document(flashcardId).get()
          .get().exists()) {
        ApiFuture<WriteResult> future = firestore.collection(PATH_NAME_FOR_FLASH_CARD_COLLECTION)
            .document(flashcardId).delete();
