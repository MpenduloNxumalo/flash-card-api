...
public class FlashCardApiController implements DeckApi, FlashCardApi {
    ...
    public ResponseEntity<OperationStatus> createDeck(Deck body) {
        ...
        try {
            return ResponseEntity
                    ...
                    .body(deckService.createDeck(body));
    ...
    public ResponseEntity<DeckResponseObject> retrieveDeckById(String deckId) {
        ...
        if (!deckIdValidationOperationStatus.isIsSuccessful()) {
            return ResponseEntity.badRequest().body(
                    new DeckResponseObject().error(deckIdValidationOperationStatus.getError())
            );
        }
        ...
        try {
            ...
            return ResponseEntity.internalServerError().body(new DeckResponseObject().error(e.getMessage()));
        } catch (NotFoundException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(
                    new DeckResponseObject().error(e.getMessage())
            );
        }
    }
    ...
    public ResponseEntity<DeckListResponseObject> retrieveDeckByName(String name) {
        ...
        try {
            return ResponseEntity.ok().body(new DeckListResponseObject().body(deckService.retrieveDeckByName(name)));
        } catch (RuntimeException exception) {
            return ResponseEntity.internalServerError().body(new DeckListResponseObject().error(exception.getMessage()));
        } catch (NotFoundException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new DeckListResponseObject().error(e.getMessage()));
        }
    }
        ...
        try {
            ...
            if (timestamp == null) {
                return ResponseEntity.internalServerError().body(
                        new OperationStatus().isSuccessful(false).error("addCardIdToDeckById method returned null")
                );
            } else if (timestamp.isEmpty()) {
                return ResponseEntity.internalServerError().body(
                        new OperationStatus().isSuccessful(false).error("addCardIdToDeckById method returned empty string")
                );
            } else {
                return ResponseEntity.ok().body(new OperationStatus().isSuccessful(true).timestamp(timestamp));
            }
        } catch (RuntimeException e) {
            return ResponseEntity.internalServerError().body(
                    new OperationStatus().error(e.getMessage())
            );
        } catch (NotFoundException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(
                    new OperationStatus().error(e.getMessage())
    ...
    @Override
    public ResponseEntity<OperationStatus> deleteCardIdFromDeckById(String deckId, String flashCardId) {
        OperationStatus flashCardIdValidationOperationStatus = ControllerUtil.validateId(flashCardId);
        OperationStatus deckIdValidationOperationStatus = ControllerUtil.validateId(deckId);
    ...
    public ResponseEntity<OperationStatusListResponseObject> deleteDeckByName(String name) {
        ...
        if (!nameValidationOperationStatus.isIsSuccessful()) {
            return ResponseEntity
                    .status(HttpStatus.NOT_FOUND)
                    .body(new OperationStatusListResponseObject().error(nameValidationOperationStatus.getError()));
        }
        ...
        try {
            ...
            if (responses.isEmpty()) {
                return ResponseEntity
                        .status(HttpStatus.NOT_FOUND)
                        .body(new OperationStatusListResponseObject().error(
                                String.format("Deck with the name: %s does not exist", name))
                        );
            }
            return ResponseEntity
                    .status(HttpStatus.OK)
        ...
        try {
            ...
            return ResponseEntity
                    ...
                    .body(new OperationStatus().isSuccessful(true)
                            .timestamp(deckService.addCardIdToDeckById(deckId, flashCardId)).flashCardId(flashCardId));
        } catch (RuntimeException e) {
            return ResponseEntity
                    .status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new OperationStatus().error(e.getMessage()));
    ...
    public ResponseEntity<FlashCardResponseObject> retrieveFlashCardById(String flashCardId) {
        ...
        if (!flashCardIdValidationOperationStatus.isIsSuccessful()) {
            return ResponseEntity
                    .status(HttpStatus.BAD_REQUEST)
                    .body(new FlashCardResponseObject().error(flashCardIdValidationOperationStatus.getError()));
        }
        ...
        try {
            ...
            return ResponseEntity
                    .status(HttpStatus.OK)
                    .body(new FlashCardResponseObject().body(Objects.requireNonNullElseGet(flashCard, FlashCard::new)));
        } catch (RuntimeException e) {
            return ResponseEntity
                    .status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<FlashCardListResponseObject> retrieveFlashCardByName(String name) {
        if (name == null || name.trim().isEmpty()) {
            return ResponseEntity
                    .status(HttpStatus.BAD_REQUEST)
                    .body(new FlashCardListResponseObject().error("Flash Card Name cannot be null or an empty string"));
        }

        try {
            return ResponseEntity
                    .status(HttpStatus.OK)
                    .body(new FlashCardListResponseObject().body(flashCardService.retrieveFlashCardByName(name)));
        } catch (RuntimeException e) {
            return ResponseEntity
                    .status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<FlashCardListResponseObject> retrieveAllFlashCardsInDeck(String deckId) {
        ...
        if (!deckIdValidationOperationStatus.isIsSuccessful()) {
            return ResponseEntity
                    .status(HttpStatus.BAD_REQUEST)
                    .body(new FlashCardListResponseObject().error(deckIdValidationOperationStatus.getError()));
        }

        try {
            return ResponseEntity
                    .status(HttpStatus.OK)
                    .body(new FlashCardListResponseObject().body(flashCardService.retrieveAllFlashCardsInDeck(deckId)));
        } catch (RuntimeException e) {
            return ResponseEntity
                    .status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new FlashCardListResponseObject().error(e.getMessage()));
    ...
    @Override
    public ResponseEntity<OperationStatus> updateFlashCardTitleById(String flashcardId, String title) {
        OperationStatus flashCardIdValidationOperationStatus = ControllerUtil.validateId(flashcardId);
        ...
        if (title == null || title.trim().isEmpty()) {
            return ResponseEntity
                    .status(HttpStatus.BAD_REQUEST)
                    .body(new OperationStatus().isSuccessful(false).error("FlashCard title cannot be null or an empty string"));
        }
        try {
            String updateTimeStamp = flashCardService.updateFlashCardTitleById(flashcardId, title);
            return ResponseEntity
                    .status(HttpStatus.OK)
                    .body(new OperationStatus().isSuccessful(true).timestamp(updateTimeStamp).flashCardId(flashcardId));
        } catch (RuntimeException e) {
            return ResponseEntity
                    .status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new OperationStatus().isSuccessful(false).error(e.getMessage()));
    ...
    public ResponseEntity<OperationStatus> updateFlashCardContentById(String id, String content) {
        ...
        if (content == null || content.trim().isEmpty()) {
            return ResponseEntity
                    .status(HttpStatus.BAD_REQUEST)
                    .body(new OperationStatus().isSuccessful(false).error("FlashCard content cannot be null or an empty string"));
        }
        try {
            String updateTimeStamp = flashCardService.updateFlashCardContentById(id, content);
            return ResponseEntity
                    .status(HttpStatus.OK)
                    .body(new OperationStatus().isSuccessful(true).timestamp(updateTimeStamp).flashCardId(id));
        } catch (RuntimeException e) {
            return ResponseEntity
                    .status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new OperationStatus().isSuccessful(false).error(e.getMessage()));
            ...
            if (flashCardDeletionTimeStamp != null) {
                ...
                if (flashCardIdRemovalFromDeckTimeStamp != null) {
                    return ResponseEntity
                            .status(HttpStatus.OK)
                            .body(new OperationStatus().isSuccessful(true)
                                    .timestamp(flashCardIdRemovalFromDeckTimeStamp)
                                    .flashCardId(flashcardId).deckId(deckId));
                } else {
                    return ResponseEntity
                            .status(HttpStatus.INTERNAL_SERVER_ERROR)
                            .body(new OperationStatus().isSuccessful(false).error("flashCardIdRemovalFromDeckTimeStamp is null"));
                }
            } else {
                return ResponseEntity
                        .status(HttpStatus.INTERNAL_SERVER_ERROR)
