...
public final class FlashCardRepositoryUtil {
    ...
    public static String updateFlashCard(Firestore firestore,String id , String fieldToBeUpdated, String update) throws NotFoundException {
        try {
            if (firestore.collection(PATH_NAME_FOR_FLASH_CARD_COLLECTION).document(id).get().get().exists()){
                Map<String, Object> updates = new HashMap<>();
                updates.put(fieldToBeUpdated, update);
                ApiFuture<WriteResult> future = firestore.collection(PATH_NAME_FOR_FLASH_CARD_COLLECTION).document(id)
                        .update(updates);
